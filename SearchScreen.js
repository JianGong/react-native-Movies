...
var SearchScreen = React.createClass({
  ...
  _urlForQueryAndPage: function(query: string, start: number): string {
    //var apiKey = API_KEYS[this.state.queryNumber % API_KEYS.length];
    if (query) {
      return (
        API_URL + 'search?' + 'q=' + encodeURIComponent(query) + '&start=' + start
      );
    } else {
      // With no query, load latest movies
      return (
        API_URL + 'in_theaters?' + 'start=' + start
      );
    }
  },
  ...
  searchMovies: function(query: string) {
    ...
    fetch(this._urlForQueryAndPage(query, 0))
      ...
      .then((responseData) => {
        ...
        resultsCache.dataForQuery[query] = responseData.subjects;
    ...
    fetch(this._urlForQueryAndPage(query, page))
      ...
      .then((responseData) => {
        ...
        if (!responseData.subjects) {
          ...
          for (var i in responseData.subjects) {
